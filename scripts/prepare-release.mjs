...
function collect(dir) {
  return readdirSync(dir, { withFileTypes: true }).flatMap(entry => {
    ...
    return entry.isDirectory() ? collect(path) : /\.(zip|exe|deb|AppImage|qlplugin|tar\.gz|pkg\.tar\.zst)$/.test(entry.name) ? [path] : [];
  });
}
const assets = collect('artifacts').sort();
for (const extension of ['.zip', '.exe', '.deb', '.AppImage', '.qlplugin', '.tar.gz', '.pkg.tar.zst']) {
