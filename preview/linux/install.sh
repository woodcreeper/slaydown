...
version=""
if command -v pacman >/dev/null; then version="$(pacman -Q sushi 2>/dev/null | awk '{print $2}')"; fi
...
if [[ ! "$major" =~ ^[0-9]+$ || "$major" -lt 46 ]]; then
  echo 'This installer supports packaged Sushi 46 or newer. Could not detect a compatible version.' >&2; exit 1
fi
if (( major >= 51 )); then
