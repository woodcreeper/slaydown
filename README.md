...
**[Download SlayDown 0.3.2](https://github.com/woodcreeper/slaydown/releases/tag/v0.3.2)** — the current preview fixes Linux menu contrast and streamlines the Sushi preview toolbar. Automatic Linux desktop setup and Folio upgrade migration are included. Space-bar previews share the reader’s appearance settings. Older v0.1.0 downloads retain the Folio name.
...
```sh
sudo apt install ./SlayDown_0.3.2_amd64.deb
```
...
```sh
chmod +x SlayDown_0.3.2_amd64.AppImage
./SlayDown_0.3.2_amd64.AppImage
```

AppImages are built on Ubuntu 22.04 and are not guaranteed to work on every distribution. If FUSE is unavailable, try `./SlayDown_0.3.2_amd64.AppImage --appimage-extract-and-run`. See [Tauri’s AppImage compatibility notes](https://v2.tauri.app/distribute/appimage/). Linux ARM64 packages are not currently provided.
